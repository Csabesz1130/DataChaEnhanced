...
class FileSlot:
    ...
    def load_file(self, filepath):
        ...
        try:
            ...
            app_logger.error(
                f"Error loading file into slot {self.slot_number}: {str(e)}"
            )
            self.clear()
            return False
    ...
    def process_file(self, voltage=-50):
        ...
        try:
            ...
            self.processor.normalized_curve, self.processor.normalized_curve_times = (
                self.processor.calculate_normalized_curve()
            )
            self.processor.average_curve, self.processor.average_curve_times = (
                self.processor.calculate_segment_average()
            )
            ...
            if result and result[0] is not None:
                ...
                app_logger.error(
                    f"Failed to generate purple curves for {self.filename}"
                )
                return False
    ...
    def _calculate_integrals(self):
        ...
        try:
            ...
            hyperpol_start, hyperpol_end = 0, min(
                200, len(self.processor.modified_hyperpol)
            )
            depol_start, depol_end = 0, min(200, len(self.processor.modified_depol))

            # Calculate hyperpol integral
            hyperpol_data = self.processor.modified_hyperpol[
                hyperpol_start:hyperpol_end
            ]
            hyperpol_times = self.processor.modified_hyperpol_times[
                hyperpol_start:hyperpol_end
            ]
            ...
            app_logger.error(
                f"Error calculating integrals for {self.filename}: {str(e)}"
            )
...
class MultiFileAnalysisWindow:
    ...
    def __init__(self, parent_app):
        ...
        # FIX: Use parent_app.master instead of parent_app directly
    ...
    def create_file_management_frame(self):
        ...
        ttk.Button(
            control_frame, text="Clear Slot", command=self.clear_current_slot
        ).pack(side="left", padx=5)
        ttk.Button(control_frame, text="Clear All", command=self.clear_all_slots).pack(
            side="left", padx=5
        )
        ...
        ttk.Entry(voltage_frame, textvariable=self.voltage_var, width=8).pack(
            side="left", padx=5
        )

        ttk.Button(
            control_frame,
            text="Process Current File",
            command=self.process_current_file,
        ).pack(side="left", padx=5)
        ttk.Button(
            control_frame, text="Process All Files", command=self.process_all_files
        ).pack(side="left", padx=5)
    ...
    def create_analysis_frame(self):
        ...
        analysis_frame = ttk.LabelFrame(
            self.window, text="Analysis Visualization", padding="5"
        )
    ...
    def create_results_frame(self):
        ...
        # Export button
        ttk.Button(
            results_frame, text="Export Results to CSV", command=self.export_results
        ).pack(pady=5)
    ...
    def load_file(self):
        ...
        if filepath:
            ...
            if slot.load_file(filepath):
                ...
                self.update_plot()
                messagebox.showinfo(
                    "Success", f"File loaded into slot {self.current_slot + 1}"
                )
            else:
    ...
    def _plot_current_file(self):
        ...
        if slot.raw_data is not None:
            self.ax.plot(
                slot.time_data * 1000, slot.raw_data, "b-", alpha=0.5, label="Raw"
            )

        # Plot filtered data
        if slot.filtered_data is not None:
            self.ax.plot(
                slot.time_data * 1000, slot.filtered_data, "g-", label="Filtered"
            )

        # Plot purple curves if processed
        if slot.is_processed and hasattr(slot.processor, "modified_hyperpol"):
            self.ax.plot(
                slot.processor.modified_hyperpol_times * 1000,
            ...
            self.ax.plot(
                slot.processor.modified_depol_times * 1000,
        ...
        for i, slot in enumerate(self.file_slots):
            if slot.is_loaded and slot.filtered_data is not None:
                self.ax.plot(
                    ...
                    slot.filtered_data,
                    color=colors[i],
                    label=f"Slot {i+1}: {slot.filename}",
                )
                plotted_any = True
...
class MultiFileAnalysisButton:
    ...
    def open_analysis_window(self):
        ...
        try:
            if (
                self.analysis_window is None
                or not self.analysis_window.window.winfo_exists()
            ):
            ...
            messagebox.showerror(
                "Error", f"Failed to open multi-file analysis:\n{str(e)}"
            )
