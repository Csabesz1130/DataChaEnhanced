...
class SignalAnalyzerApp:
    ...
    def data(self, value):
        ...
        if self._data is not None:
            ...
            app_logger.debug(
                f"Cleaning up old data array: {old_size / 1024 / 1024:.1f} MB"
            )
            del self._data
        ...
        if value is not None:
            ...
            app_logger.debug(
                f"New data stored: {self._data.shape}, {new_size / 1024 / 1024:.1f} MB"
            )
        else:
            self._data = None
    ...
    def initialize_curve_fitting(self):
        ...
        try:
            if (
                hasattr(self, "fig")
                and hasattr(self, "ax")
                and hasattr(self, "action_potential_tab")
            ):
    ...
    def fix_window_sizing(self):
        ...
        try:
            ...
            app_logger.info(
                "Window sizing fixed with zoom controls (Ctrl+/-, F11, Ctrl+0)"
            )
    ...
    def cleanup_matplotlib_figures(self):
        ...
        try:
            ...
            if len(figure_numbers) > 3:  # Keep max 3 figures
                app_logger.debug(
                    f"Cleaning up {len(figure_numbers)} matplotlib figures"
                )
    ...
    def force_cleanup(self):
        ...
        try:
            ...
            try:
                ...
                memory_mb = process.memory_info().rss / 1024 / 1024
                app_logger.info(
                    f"Force cleanup: collected {total_collected} objects, current usage: {memory_mb:.1f} MB"
                )
            except ImportError:
    ...
    def setup_menubar(self):
        ...
        analysis_menu.add_command(
            label="View History", command=self.show_analysis_history
        )
        analysis_menu.add_command(
            label="Export Purple Curves", command=self.on_export_purple_curves
        )
    ...
    def run_ai_analysis(self):
        ...
        if hasattr(self, "tabs") and "ai_analysis" in self.tabs:
            ...
            for i, tab_name in enumerate(
                ["filter", "analysis", "view", "action_potential", "ai_analysis"]
            ):
            ...
            messagebox.showwarning(
                "AI Not Available", "AI Analysis is not available in this installation."
            )
        ...
        try:
            ...
            if has_tracker:
                ...
                for curve_type, curve_data in self.point_tracker.curve_data.items():
                    has_data = (
                        curve_data["data"] is not None and len(curve_data["data"]) > 0
                    )
            ...
            processor_valid = (
                processor_exists and self.action_potential_processor is not None
            )

            # Check show_points setting in UI
            has_ui_control = hasattr(self.action_potential_tab, "show_points")
            ui_points_enabled = (
                has_ui_control and self.action_potential_tab.show_points.get()
            )
            ...
            if result:
                if has_tracker:
                    ...
                    messagebox.showinfo(
                        "Point Tracking", "Point tracking has been force-enabled."
                    )
                else:
                    messagebox.showerror(
                        "Error",
                        "Cannot enable point tracking - tracker not initialized.",
                    )
            ...
            messagebox.showerror(
                "Debug Error", f"Error diagnosing point tracking: {str(e)}"
            )
    ...
    def debug_processor(self):
        ...
        try:
            ...
            processor_valid = (
                has_processor and self.action_potential_processor is not None
            )
            ...
            if processor_valid:
                ...
                message += (
                    f"Has modified_depol: {hasattr(processor, 'modified_depol')}\n"
                )
    ...
    def setup_toolbar(self):
        ...
        ttk.Button(file_frame, text="Load Data", command=self.load_data).pack(
            side="left", padx=2
        )
        ttk.Button(file_frame, text="Export Data", command=self.export_data).pack(
            side="left", padx=2
        )
        ttk.Button(file_frame, text="Export Figure", command=self.export_figure).pack(
            side="left", padx=2
        )

        ttk.Separator(self.toolbar_frame, orient="vertical").pack(
        ...
        ttk.Button(
            export_frame, text="View History", command=self.show_analysis_history
        ...
        self.separate_plots_btn = ttk.Button(
            plots_frame, text="Separate Plots ▼", command=self.show_plot_menu
        )
        ...
        ttk.Button(
            updates_frame, text="Check for Updates", command=self.check_for_updates
        ).pack(side="left", padx=2)
        ...
        self.plot_menu.add_command(
            ...
            command=lambda: self.window_manager.open_baseline_window(
                preserve_main=True
            ),
        )
        self.plot_menu.add_command(
            label="Normalization",
            command=lambda: self.window_manager.open_normalization_window(
                preserve_main=True
            ),
        )
        self.plot_menu.add_command(
            label="Integration",
            command=lambda: self.window_manager.open_integration_window(
                preserve_main=True
            ),
        )
        ...
        debug_button = ttk.Button(
            self.toolbar_frame, text="Debug Processor", command=self.debug_processor
        )
    ...
    def show_plot_menu(self, event=None):
        ...
        y = (
            self.separate_plots_btn.winfo_rooty()
            + self.separate_plots_btn.winfo_height()
        )
                ...
                for i, tab_id in enumerate(self.notebook.tabs()):
                    if self.notebook.tab(tab_id, "text") == tab_text:
                        ...
                        if tab_key == "filter":
                            ...
                            importlib.reload(ft_module)
                            new_tab = ft_module.FilterTab(
                                self.notebook, self.on_filter_change
                            )
                        elif tab_key == "analysis":
                            import importlib
                            import src.gui.analysis_tab as at_module
                            ...
                            importlib.reload(vt_module)
                            new_tab = vt_module.ViewTab(
                                self.notebook, self.on_view_change
                            )
                        elif tab_key == "ai_analysis":
                            import importlib
                            import src.gui.ai_analysis_tab as aat_module
    ...
    def save_current_plot_limits(self):
        ...
        try:
            if hasattr(self, "ax"):
                ...
                app_logger.debug(
                    f"Saved plot limits: x={self.saved_plot_limits['xlim']}, y={self.saved_plot_limits['ylim']}"
                )
        except Exception as e:
    ...
    def restore_plot_limits(self):
        ...
        try:
            if (
                hasattr(self, "ax")
                and self.saved_plot_limits["auto_restore"]
                and self.saved_plot_limits["xlim"] is not None
                and self.saved_plot_limits["ylim"] is not None
            ):

                self.ax.set_xlim(self.saved_plot_limits["xlim"])
                self.ax.set_ylim(self.saved_plot_limits["ylim"])
                app_logger.debug(
                    f"Restored plot limits: x={self.saved_plot_limits['xlim']}, y={self.saved_plot_limits['ylim']}"
                )
                return True
        except Exception as e:
    ...
    def toggle_auto_restore_limits(self, enable=None):
        ...
        if enable is None:
            self.saved_plot_limits["auto_restore"] = not self.saved_plot_limits[
                "auto_restore"
            ]
        else:
        ...
        app_logger.info(
            f"Auto-restore plot limits: {self.saved_plot_limits['auto_restore']}"
        )
    ...
    def setup_plot_context_menu(self):
        ...
        try:
            ...
            def on_right_click(event):
                ...
                auto_restore_text = (
                    "Disable Auto-Zoom Restore"
                    if self.saved_plot_limits["auto_restore"]
                    else "Enable Auto-Zoom Restore"
                )
                ...
                context_menu.add_command(
                    label="Save Current Zoom", command=self.save_current_plot_limits
                )
                ...
                context_menu.add_command(
                    label="Clear Saved Zoom", command=self.clear_saved_limits
                )
    ...
    def on_show_points_toggle(self):
        ...
        try:
            ...
            if hasattr(app, "point_tracker"):
                ...
                app_logger.debug(
                    f"Directly updated point tracker show_points to {show_points}"
                )
        except Exception as e:
    ...
    def update_point_tracking(self, enable_annotations=False):
        ...
        if hasattr(processor, "orange_curve") and processor.orange_curve is not None:
            ...
            self.point_tracker.curve_data["orange"]["times"] = getattr(
                processor, "orange_curve_times", None
            )

        if (
            hasattr(processor, "normalized_curve")
            and processor.normalized_curve is not None
        ):
            self.point_tracker.curve_data["blue"]["data"] = processor.normalized_curve
            self.point_tracker.curve_data["blue"]["times"] = getattr(
                processor, "normalized_curve_times", None
            )
        ...
        if hasattr(processor, "average_curve") and processor.average_curve is not None:
            ...
            self.point_tracker.curve_data["magenta"]["times"] = getattr(
                processor, "average_curve_times", None
            )
        ...
        if (
            ...
            self.point_tracker.curve_data["purple_hyperpol"]["times"] = getattr(
                processor, "modified_hyperpol_times", None
            )
        ...
        if (
            ...
            self.point_tracker.curve_data["purple_depol"]["times"] = getattr(
                processor, "modified_depol_times", None
            )
    ...
    def setup_plot_interaction(self):
        ...
        def on_select(xmin, xmax):
            ...
            try:
                ...
                time_range = (
                    self.action_potential_processor.modified_hyperpol_times[-1]
        ...
        self.span_selector = SpanSelector(
            ...
            "horizontal",
            useblit=True,
            props=dict(alpha=0.3, facecolor="lightblue", edgecolor="blue"),
            interactive=True,
    ...
    def toggle_span_selectors(self, visible):
        ...
        if hasattr(self, "span_selector"):
            try:
                ...
                app_logger.debug(
                    f"All span selectors visibility set to {active} (requested: {visible}, mode: {display_mode})"
                )
    ...
    def on_remove_spikes(self):
        ...
        try:
            ...
            if (
                not hasattr(self, "action_potential_processor")
                or self.action_potential_processor is None
            ):
                messagebox.showwarning(
                    "No Processor", "Please load data and run analysis first."
                )
                return
            ...
            if not hasattr(processor, "orange_curve") or processor.orange_curve is None:
                messagebox.showwarning(
                    "No Data", "The processor doesn't have any curve data to process."
                )
                return
    ...
    def _on_canvas_double_click(self, event):
        ...
        try:
            app_logger.info("🖱️ Canvas double-clicked - opening file dialog")
            self.load_data()
        except Exception as e:
            app_logger.error(f"Error handling canvas double-click: {e}")
    ...
    def setup_tabs(self):
        """Setup the control tabs with lazy loading"""
        ...
        ttk.Label(
            self._filter_placeholder,
            text="Filters (Click to load)",
            justify="center",
        ...
        ttk.Label(
            ...
            text="View (Click to load)",
            justify="center",
        ).pack(padx=20, pady=20)
        self.notebook.add(self._view_placeholder, text="View")

        # Create Action Potential tab immediately as it's commonly used
        self.action_potential_tab = ActionPotentialTab(
            self.notebook, self.on_action_potential_analysis
        )
        self.tabs["action_potential"] = self.action_potential_tab
    ...
    def _load_filter_tab(self):
        ...
        try:
            ...
            for widget in self._filter_placeholder.winfo_children():
                widget.destroy()
            ttk.Label(
                self._filter_placeholder,
    ...
    def _load_analysis_tab(self):
        ...
        try:
            ...
            # Show error in placeholder
            for widget in self._analysis_placeholder.winfo_children():
                widget.destroy()
            ttk.Label(
                self._analysis_placeholder,
    ...
    def _load_view_tab(self):
        ...
        try:
            ...
            # Show error in placeholder
            for widget in self._view_placeholder.winfo_children():
                widget.destroy()
            ttk.Label(
                self._view_placeholder,
                text=f"Failed to load View tab:\n{str(e)}",
                foreground="red",
            ).pack(padx=20, pady=20)
    ...
    def _find_closest_point(self, x_ms, y_pA):
        ...
        if (
            not hasattr(self, "orange_curve")
            or self.orange_curve is None
            or not hasattr(self, "orange_curve_times")
            or self.orange_curve_times is None
            or not hasattr(self.action_potential_tab, "average_display_mode")
            or self.action_potential_tab.average_display_mode.get() != "all_points"
        ):
            return None, None
        ...
        time_threshold = max(
            0.0005, 0.01 * x_range
        )  # 1% of time range or at least 0.5ms
        amp_threshold = max(
            10.0, 0.05 * y_range
        )  # 5% of amplitude range or at least 10 pA

        # Log detection details for debugging
        app_logger.debug(
            f"Point search: x={x_sec:.5f}s, closest={self.orange_curve_times[closest_idx]:.5f}s, diff={min_diff:.5f}/{time_threshold:.5f}"
        )
        app_logger.debug(
            f"Amplitude: y={y_pA:.1f}, closest={self.orange_curve[closest_idx]:.1f}, diff={abs(self.orange_curve[closest_idx] - y_pA):.1f}/{amp_threshold:.1f}"
        )

        # Check if closest point is within thresholds
        if (
            min_diff <= time_threshold
            and abs(self.orange_curve[closest_idx] - y_pA) <= amp_threshold
        ):
    ...
    def on_mouse_move(self, event):
        ...
        if (
            hasattr(self, "orange_curve")
            and hasattr(self.action_potential_tab, "average_display_mode")
            and self.action_potential_tab.average_display_mode.get() == "all_points"
        ):
    ...
    def on_export_purple_curves(self):
        ...
        results_dict = (
            self.action_potential_tab.get_results_dict()
        )  # or whatever your code uses
        if not results_dict:
    ...
    def setup_interactive_ranges(self):
        ...
        app_logger.debug(
            "Interactive integration ranges initialized with improved sensitivity"
        )
    ...
    def on_depol_span_select(self, xmin, xmax):
        ...
        if (
            ...
            if (
                not hasattr(self.action_potential_tab, "processor")
                or self.action_potential_tab.processor is None
            ):
                return
    ...
    def load_data(self):
        ...
        try:
            ...
            app_logger.info(
                f"  time_data: {self.time_data is not None} (length: {len(self.time_data) if self.time_data is not None else 0})"
            )
            app_logger.info(
                f"  data: {self.data is not None} (length: {len(self.data) if self.data is not None else 0})"
            )
            app_logger.info(
                f"  filtered_data: {self.filtered_data is not None} (length: {len(self.filtered_data) if self.filtered_data is not None else 0})"
            )
            filename = os.path.basename(filepath)
            ...
            app_logger.info(
                f"File loaded successfully. Data size: {data_size_mb:.1f} MB"
            )
            ...
            if hasattr(self, "history_manager") and hasattr(
                ...
                file_history = [
                    entry for entry in history if entry["filename"] == filename
                ]
    ...
    def on_analysis_update(self, analysis_params):
        ...
        try:
            self.analysis_tab.analyze_data(
                self.filtered_data, self.time_data, analysis_params
            )
        except Exception as e:
    ...
    def on_display_mode_change(self, *args):
        ...
        try:
            ...
            app_logger.error(
                f"Error updating span selectors for display mode: {str(e)}"
            )
    ...
    def on_action_potential_analysis(self, params):
        ...
        try:
            app_logger.debug(
                f"Starting action potential analysis with params: {params}"
            )
            self.clear_saved_limits()
            # Initialize the processor
            self.action_potential_processor = ActionPotentialProcessor(
                self.filtered_data, self.time_data, params
            )
            ...
            (
                ...
                auto_optimize_starting_point=params.get(
            ...
            if (
                modified_hyperpol is None
                or modified_depol is None
                or modified_hyperpol_times is None
                or modified_depol_times is None
            ):
            ...
            purple_results = (
                self.action_potential_processor.calculate_purple_integrals()
            )
            ...
            app_logger.debug(
                f"Analysis complete - action_potential_processor reference is {self.action_potential_processor is not None}"
            )
            ...
            try:
                ...
                if (
                    hasattr(self, "curve_fitting_panel")
                    and self.curve_fitting_panel
                    and hasattr(self, "action_potential_processor")
                    and self.action_potential_processor
                ):
            ...
            if self.history_manager:
                self.history_manager.add_entry(
                    filename=self.current_file, results=results, analysis_type="manual"
                )
    ...
    def update_curve_fitting_after_processor_change(self):
        ...
        try:
            ...
            app_logger.error(
                f"Error updating curve fitting after processor change: {str(e)}"
            )
    ...
    def plot_action_potential(self, processed_data, time_data):
        ...
        try:
            ...
            self.ax.plot(
                time_data, processed_data, "g-", linewidth=1.5, label="Processed"
            )
    ...
    def show_file_history_dialog(self, filename):
        ...
        if not hasattr(self, "history_manager") or not hasattr(
            self.history_manager, "history_entries"
        ):
            return
        ...
        progress = ttk.Progressbar(
            progress_frame, length=300, mode="determinate", value=100
        )
    ...
    def update_plot(self, view_params=None, force_full_range=False):
        ...
        try:
            ...
            if show_original and self.data is not None:
                time_plot, data_plot = self._downsample_for_plot(
                    self.time_data, self.data
                )
                self.ax.plot(
                    time_plot, data_plot, "b-", label="Original", alpha=0.7, linewidth=1
                )
                del time_plot, data_plot  # Clean up immediately

            if show_filtered and self.filtered_data is not None:
                time_plot, filtered_plot = self._downsample_for_plot(
                    self.time_data, self.filtered_data
                )
                self.ax.plot(
                    time_plot, filtered_plot, "r-", label="Filtered", linewidth=2
                )
            ...
            if force_full_range:
                ...
                if self.time_data is not None and self.data is not None:
                    ...
                    self.ax.set_ylim(
                        np.min(self.data) * 1.05, np.max(self.data) * 1.05
                    )  # Add 5% padding
                    limits_applied = True
    ...
    def _plot_curve_if_enabled(
        self, x_data, y_data, enabled, *plot_args, **plot_kwargs
    ):
    ...
    def _plot_processed_curve(self, processed_data, time_data):
        ...
        try:
            ...
            if display_mode in ["points", "all_points"]:
                ...
                time_plot, data_plot = self._downsample_for_plot(
                    time_ms, processed_data, max_points=5000
                )
                self.ax.scatter(
                    time_plot,
                    data_plot,
    ...
    def _plot_average_curve(self, orange_curve, orange_times):
        ...
        try:
            ...
            if display_mode in ["points", "all_points"]:
                time_plot, data_plot = self._downsample_for_plot(
                    time_ms, orange_curve, max_points=5000
                )
                self.ax.scatter(
                    time_plot,
                    data_plot,
    ...
    def _plot_normalized_curve(self, normalized_curve, normalized_times):
        ...
        try:
            ...
            if display_mode in ["points", "all_points"]:
                time_plot, data_plot = self._downsample_for_plot(
                    time_ms, normalized_curve, max_points=5000
                )
                self.ax.scatter(
                    time_plot,
                    data_plot,
    ...
    def _plot_averaged_normalized_curve(self, average_curve, average_curve_times):
        ...
        try:
            display_mode = (
                self.action_potential_tab.averaged_normalized_display_mode.get()
            )
            ...
            if display_mode in ["points", "all_points"]:
                time_plot, data_plot = self._downsample_for_plot(
                    time_ms, average_curve, max_points=5000
                )
                self.ax.scatter(
                    time_plot,
                    data_plot,
    ...
    def _plot_purple_curves_with_ranges(
        self, display_options, integration_ranges, intervals, show_points
    ):
        """Plot purple curves with integration ranges and memory optimization"""
        try:
            has_purple_curves = (
                hasattr(self.action_potential_processor, "modified_hyperpol")
                and hasattr(self.action_potential_processor, "modified_depol")
            ...
            if display_mode in ["line", "all_points"]:
                ...
                hyp_time_plot, hyp_data_plot = self._downsample_for_plot(
                    hyperpol_times * 1000, hyperpol
                )
                dep_time_plot, dep_data_plot = self._downsample_for_plot(
                    depol_times * 1000, depol
                )
            ...
            if display_mode in ["points", "all_points"]:
                ...
                hyp_time_plot, hyp_data_plot = self._downsample_for_plot(
                    hyperpol_times * 1000, hyperpol, max_points=3000
                )
                dep_time_plot, dep_data_plot = self._downsample_for_plot(
                    depol_times * 1000, depol, max_points=3000
                )
            ...
            self._add_integration_range_visualizations(
                integration_ranges, hyperpol_times, depol_times
            )
            ...
            if show_points and hasattr(self, "plot_regression_lines"):
                ...
                if regression_interval:
                    ...
                    self.plot_regression_lines(
                        depol, depol_times, regression_interval, color="red", alpha=0.8
                    )

        except Exception as e:
            app_logger.error(f"Error plotting purple curves: {str(e)}")

    def _add_integration_range_visualizations(
        self, integration_ranges, hyperpol_times, depol_times
    ):
        ...
        try:
            ...
            if "hyperpol" in integration_ranges and hyperpol_times is not None:
                ...
                if 0 <= start_idx < len(hyperpol_times) and 0 < end_idx <= len(
                    hyperpol_times
                ):
                    self.ax.axvspan(
                        hyperpol_times[start_idx] * 1000,
            ...
            if "depol" in integration_ranges and depol_times is not None:
                ...
                if 0 <= start_idx < len(depol_times) and 0 < end_idx <= len(
                    depol_times
                ):
                    self.ax.axvspan(
                        depol_times[start_idx] * 1000,
    ...
    def _update_span_selectors(self, show_points, integration_ranges):
        ...
        try:
            ...
            active_mode = False
            if hasattr(self.action_potential_tab, "modified_display_mode"):
                active_mode = self.action_potential_tab.modified_display_mode.get() in [
                    "line",
                    "all_points",
                ]
            ...
            if (
                enable_spans
                and hasattr(self, "hyperpol_span")
                and hasattr(self, "depol_span")
            ):
    ...
    def _position_span_selectors(self, integration_ranges):
        ...
        try:
            if not (
                hasattr(self.action_potential_processor, "modified_hyperpol_times")
                and hasattr(self.action_potential_processor, "modified_depol_times")
            ):
                return
            ...
            if "hyperpol" in integration_ranges and hyperpol_times is not None:
                ...
                if 0 <= start_idx < len(hyperpol_times) and 0 < end_idx <= len(
                    ...
                    if (
                        not hasattr(self, "prev_hyperpol_extents")
                        or self.prev_hyperpol_extents != new_extents
                    ):
            ...
            if "depol" in integration_ranges and depol_times is not None:
                ...
                if 0 <= start_idx < len(depol_times) and 0 < end_idx <= len(
                    ...
                    if (
                        not hasattr(self, "prev_depol_extents")
                        or self.prev_depol_extents != new_extents
                    ):
    ...
    def update_plot_with_processed_data(
        ...
        try:
            ...
            view_params = self.view_tab.get_view_params()
            display_options = self.action_potential_tab.get_parameters().get(
                "display_options", {}
            )
            ...
            app_logger.debug(
                f"Updating plot with integration ranges: {integration_ranges}"
            )
            ...
            if processed_data is not None and display_options.get(
                "show_processed", True
            ):
            ...
            self._plot_purple_curves_with_ranges(
                display_options, integration_ranges, intervals, show_points
            )
            ...
            # Apply explicit view parameters
                ...
                self.ax.set_xlim(
                    view_params["t_min"] * 1000, view_params["t_max"] * 1000
                )
                limits_applied = True
    ...
    def _apply_auto_scale_limits(self, display_options):
        ...
        try:
            ...
            if (
                display_options.get("show_processed", True)
                and hasattr(self, "processed_data")
                and self.processed_data is not None
            ):
            ...
            if (
                display_options.get("show_average", True)
                and hasattr(self, "orange_curve")
                and self.orange_curve is not None
            ):
            ...
            if (
                display_options.get("show_normalized", True)
                and hasattr(self, "normalized_curve")
                and self.normalized_curve is not None
            ):
            ...
            if (
                display_options.get("show_modified", True)
                and hasattr(self, "action_potential_processor")
                and self.action_potential_processor
            ):
            ...
            if all_y_data and all_x_data:
                ...
                app_logger.debug(
                    f"Auto-scale applied: x=({x_min:.1f}, {x_max:.1f}), y=({y_min:.1f}, {y_max:.1f})"
                )
            else:
        ...
        try:
            ...
            if filepath:
                ...
                results_dict = {
                    ...
                    "purple_integral_value": getattr(
                        processor, "purple_integral_value", "N/A"
                    ),
                }
                ...
                with open(filepath, "w", newline="") as f:
                    ...
                    writer.writerow(
                        ["Overall Integral (pC):", results_dict["integral_value"]]
                    )
                    writer.writerow([])
                    ...
                    writer.writerow(
                        ["Hyperpol Integral:", results_dict["hyperpol_area"]]
                    )
                    writer.writerow(["Index", "Hyperpol_pA", "Hyperpol_time_ms"])

                    if hasattr(processor, "modified_hyperpol") and hasattr(
                    ...
                    if hasattr(processor, "modified_depol") and hasattr(
                        ...
                        for i in range(len(depol)):
                            writer.writerow(
                                [i + 1, f"{depol[i]:.7f}", f"{depol_times[i]*1000:.7f}"]
                            )
                    writer.writerow([])
                    ...
                    writer.writerow(
                        [
                            "Purple Integral Summary:",
                            results_dict["purple_integral_value"],
                        ]
                    )
    ...
    def on_closing(self):
        ...
        try:
            ...
            if "excel_learning" in self.tabs and hasattr(
