...
class ExcelLearningTab:
    ...
    def initialize_background_processor(self):
        ...
        try:
            ...
            from src.ai_excel_learning.background_processor_simple import SimpleBackgroundProcessor, TaskStatus, NotificationType
            ...
            try:
                from src.ai_excel_learning.background_processor import BackgroundProcessor
    ...
    def setup_ui(self):
        ...
        # Title
        title_label = ttk.Label(main_frame, text="Excel AI Learning Background Tasks", 
                               font=('Arial', 14, 'bold'))
    ...
    def setup_submit_tab(self):
        ...
        file_frame = ttk.LabelFrame(submit_frame, text="Excel File Selection", padding=10)
        ...
        self.file_path_entry = ttk.Entry(file_path_frame, textvariable=self.file_path_var, width=50)
        self.file_path_entry.pack(side='left', fill='x', expand=True)
        
        ttk.Button(file_path_frame, text="Browse", command=self.browse_file).pack(side='right', padx=(5, 0))
        
        # Task configuration section
        config_frame = ttk.LabelFrame(submit_frame, text="Task Configuration", padding=10)
        config_frame.pack(fill='x', padx=10, pady=10)
        
        # Task type
        ttk.Label(config_frame, text="Learning Type:").pack(anchor='w')
        self.task_type_var = tk.StringVar(value="full_pipeline")
        task_type_combo = ttk.Combobox(config_frame, textvariable=self.task_type_var, state='readonly')
        ...
        ttk.Radiobutton(priority_frame, text="Low", variable=self.priority_var, value=1).pack(side='left')
        ttk.Radiobutton(priority_frame, text="Medium", variable=self.priority_var, value=3).pack(side='left', padx=10)
        ttk.Radiobutton(priority_frame, text="High", variable=self.priority_var, value=5).pack(side='left')
        
        # Submit button
        submit_frame_buttons = ttk.Frame(submit_frame)
        submit_frame_buttons.pack(fill='x', padx=10, pady=20)
        
        self.submit_button = ttk.Button(submit_frame_buttons, text="Submit Learning Task", 
                                       command=self.submit_task)
        self.submit_button.pack(side='left')
        
        ttk.Button(submit_frame_buttons, text="Clear", command=self.clear_submit_form).pack(side='left', padx=10)
        
        # Status display
        self.submit_status_var = tk.StringVar()
        ttk.Label(submit_frame, textvariable=self.submit_status_var, foreground='blue').pack(pady=10)
    ...
    def setup_monitor_tab(self):
        ...
        ttk.Button(control_frame, text="Refresh", command=self.refresh_tasks).pack(side='left')
        ttk.Button(control_frame, text="Clear Completed", command=self.clear_completed_tasks).pack(side='left', padx=10)
        ttk.Button(control_frame, text="Stop All", command=self.stop_all_tasks).pack(side='left', padx=10)
        ...
        details_scrollbar = ttk.Scrollbar(details_frame, orient='vertical', command=self.task_details_text.yview)
    ...
    def setup_results_tab(self):
        ...
        ttk.Button(results_control_frame, text="Refresh Results", command=self.refresh_results).pack(side='left')
        ttk.Button(results_control_frame, text="Export Results", command=self.export_results).pack(side='left', padx=10)
        ttk.Button(results_control_frame, text="Clear Results", command=self.clear_results).pack(side='left', padx=10)
        
        # Results list
        results_list_frame = ttk.LabelFrame(results_frame, text="Completed Tasks", padding=10)
        results_list_frame.pack(fill='both', expand=True, padx=10, pady=10)
        
        # Create treeview for results
        result_columns = ('Task ID', 'File', 'Type', 'Status', 'Completed', 'Result Size')
        self.results_tree = ttk.Treeview(results_list_frame, columns=result_columns, show='headings', height=8)
        ...
        # Result details
        result_details_frame = ttk.LabelFrame(results_frame, text="Result Details", padding=10)
        ...
        result_details_scrollbar = ttk.Scrollbar(result_details_frame, orient='vertical', command=self.result_details_text.yview)
    ...
    def submit_task(self):
        ...
        try:
            ...
            self.submit_status_var.set(f"Task submitted successfully! Task ID: {task_id}")
    ...
    def refresh_tasks(self):
        ...
        try:
            ...
            for task_id, task in all_tasks.items():
                ...
                created_str = task.created_at.strftime("%H:%M:%S") if task.created_at else "N/A"
    ...
    def clear_completed_tasks(self):
        ...
        try:
            all_tasks = self.background_processor.get_all_tasks()
            completed_tasks = [task_id for task_id, task in all_tasks.items() 
    ...
    def stop_all_tasks(self):
        ...
        if messagebox.askyesno("Confirm", "Are you sure you want to stop all active tasks?"):
            try:
                # This would require adding a stop_all_tasks method to BackgroundProcessor
                # For now, we'll just show a message
                messagebox.showinfo("Info", "Stop all functionality not yet implemented")
    ...
    def show_task_details(self, task):
        ...
        started_str = task.started_at.strftime('%Y-%m-%d %H:%M:%S') if task.started_at else 'N/A'
        completed_str = task.completed_at.strftime('%Y-%m-%d %H:%M:%S') if task.completed_at else 'N/A'
        metadata_str = json.dumps(task.metadata, indent=2) if task.metadata else 'None'
        error_str = task.error if task.error else 'None'
        result_summary = self.format_result_summary(task.result) if task.result else 'None'
...
"""
    ...
    def format_result_summary(self, result):
        ...
        try:
            ...
            if 'excel_analysis' in result:
                ...
                summary.append(f"Excel Analysis: {len(analysis.get('sheets', []))} sheets analyzed")
            
            if 'formula_learning' in result:
                formulas = result['formula_learning']
                summary.append(f"Formula Learning: {len(formulas.get('learned_formulas', []))} formulas learned")
            
            if 'chart_learning' in result:
                charts = result['chart_learning']
                summary.append(f"Chart Learning: {len(charts.get('learned_charts', []))} charts learned")
    ...
    def refresh_results(self):
        ...
        try:
            ...
            # Get completed tasks
            all_tasks = self.background_processor.get_all_tasks()
            completed_tasks = {task_id: task for task_id, task in all_tasks.items() 
            ...
            for task_id, task in completed_tasks.items():
                ...
                completed_str = task.completed_at.strftime("%H:%M:%S") if task.completed_at else "N/A"
    ...
    def show_result_details(self, task):
        ...
        try:
            completed_str = task.completed_at.strftime('%Y-%m-%d %H:%M:%S') if task.completed_at else 'N/A'
            result_str = json.dumps(task.result, indent=2) if task.result else 'No result available'
...
"""
    ...
    def on_notification(self, notification):
        ...
        try:
            ...
            app_logger.info(f"Notification received: {notification.notification_type.value} - {notification.message}")
